...
# Instalação do AcodeX Server (CORRIGIDO PARA USAR NPM)
echo -ne "${CIANO}AcodeX - Terminal... ${RESET}"
if npm install -g acodex-server > /dev/null 2>&1; then 
    echo -e "${VERDE}OK${RESET}"
else 
    echo -e "${VERMELHO}Erro (Tente: npm i -g acodex-server)${RESET}"
fi
...
echo -e "${VERDE}[✓]${RESET} AcodeX Corrigido (NPM)"
echo " "
...
if [ $? -eq 0 ]; then 
    ...
    echo -e "${VERMELHO}Erro (Manual req.)${RESET}"
fi
